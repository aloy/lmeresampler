...
* `boot(model, type)`
* `boot.parametric(y,X,Z,beta)` is for the general parametric.
* `boot.residual` is for the residual.
* `boot.case` is for the cases.
* `boot.cgr` is for the semiparametric.
* `boot.reb(residuals, reb_type=(0,1,2))` will be the generalized REB function where it is passed a vector of residuals and the type of Bootstrap we want. `reb_type=0` is REB, `reb_type=1` is REB/1, and `reb_type=2` is REB/2.
