...
The semi-parametric (residual) bootstrap algorithm implemented was described by Carpenter, Goldstein and Rasbash (2003). The algorithm is outlined below:
...
The wild bootstrap exists for various types of models, but was first discussed in the context of hierarchical models by Modugno and Giannerini (2015). This bootstrap makes no distribution assumptions and allows for heteroskedasticity, which is often useful for economists. The heteroskedasticity is accounted for using the Heteroskedasticity Consistent Covariance Matrix Estimator (HCCME), and the two most commonly used flavors of these HCCMEs are HC2 and HC3. These can be set in a call to `bootstrap()` using `hccme = "hc2"` or `hccme = "hc3"`. The wild bootstrap also draws bootstrap samples for the errors from what is known as an auxiliary distribution. The two versions of this discussed in the paper, F1 and F2, can be set in a `bootstrap()` call using `aux.dist = "f1"` or `aux.dist = "f2"`. For more information on HCCMEs and the purpose and structure of the auxiliary distributions, please refer to Modugno and Giannerini's paper, which is cited in the References section.
...
The newest version of `lmeresampler` comes equipped with its own `summary()`, `print()`, `confint()` and `plot()` functions, all of which follow the syntax of generic `summary()`, `print()`, `confint()` and `plot()` functions. In order for these functions to operate as necessary, the output of the `bootstrap()` function has been updated to be an object of class `lmeresamp`. See the next sections for some examples.

## The `summary()` function

The syntax of the `lmeresampler` `summary()` function is similar to the default `summary()` function. The object to be summarized must be the object returned by the `bootstrap()` call, an `lmeresamp` object.
...
Finally, like the previous functions, the `plot()` function follows the same syntax as a default `plot()` function. In lmeresampler, this function creates a density plot of bootstrap estimates for a default (the first covariate, often the intercept) or specified variable. The bottom of the graph also contains 66% and 95% percentile intervals for the covariate. 
