...
* xxtensions for both users and future developers of the package.
...
```{r}
print.lmeresamp(lmer_reb_boot0)

# print.lmeresamp(lmer_reb_boot1, ci = TRUE)
```

## The `confint()` Function

Similarly, the syntax of the `lmeresampler` `confint()` function is similar to the default `confint()` function. The object for which confidence are calculated must be the `lmeresamp` object returned by the `bootstrap()` call. The type of confidence interval may be specified with `method`, for which possible values include: `"norm"` (normal-t interval), `"boot-t"` (bootstrap-t interval using output from the `bootstrap()` call), and `"all"` (all of aforementioned methods, and the default if `method` is unspecified). The level at which the confidence interval is to be computed may be specified with `level`, which defaults to `0.95` if unspecified. 

For those interested, it is worth noting that the bootstrap-t interval is an approximation, and is thus less statistically sound than the other intervals. This is because the algorithm calculates the interval using estimates of the standard errors of the variance components of the model, which while possible to calculate, are often unreliable. We use these estimates to provide the bootstrap-t interval for curious users, though we recommend that one of the other methods be used in situations where precise values are necessary. For more information about how these random effects standard errors are calculated, see Ben Bolker's (one of the authors of, and the current maintainer of, `lme4`) two posts (whose knowledge and linear algebra we followed closely) on the topic in the References section below.

## Examples

```{r, message = FALSE, warning = FALSE}
confint(lmer_par_boot) # all ci types, 0.95 confidence level

confint(lmer_par_boot, method = "norm", level= 0.90)
```
...
tic()
b_parallel2 <- foreach(B = rep(500, 2), .combine = combine, .packages = c("lmeresampler", "lme4", "purrr", "dplyr")) %dopar% {
  bootstrap(vcmodA, .f = fixef, type = "parametric", B = B)
}
toc()
