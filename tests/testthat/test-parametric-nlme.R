...
test_that("two-level additive random intercept model", {
  ...
  vcmodA <- lme(
    mathAge11 ~ mathAge8 + gender + class,
    random = ~ 1 | school,
    data = jsp728
  )
  ...
  boo <- parametric_bootstrap(model = vcmodA, .f = mySumm, B = nsim)
...
test_that("two-level random intercept model without interaction", {
  ...
  rimod <- lme(
    mathAge11 ~ mathAge8c + gender + class,
    random = ~ 1 | school,
    data = jsp728
  )

  orig.stats <- mySumm(rimod)
  boo <- parametric_bootstrap(model = rimod, .f = mySumm, B = nsim)
...
test_that("two-level random intercept model with interaction", {
  ...
  vcmodC <- lme(
    mathAge11 ~ mathAge8 * schoolMathAge8 + gender + class,
    random = ~ 1 | school,
    data = jsp728
  )

  orig.stats <- mySumm(vcmodC)
  boo <- parametric_bootstrap(model = vcmodC, .f = mySumm, B = nsim)
...
test_that("two-level random coefficient model with interaction", {
  ...
  rcmod <- lme(
    mathAge11 ~ mathAge8c * schoolMathAge8 + gender + class,
    random = ~ mathAge8c | school,
    data = jsp728
  )

  orig.stats <- mySumm(rcmod)
  boo <- parametric_bootstrap(model = rcmod, .f = mySumm, B = nsim)
