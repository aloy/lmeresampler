...
case_bootstrap.merMod <- function(
  model,
  .f,
  B,
  resample,
  ...
  if (length(clusters) != length(resample)) {
    stop(
      "'resample' is not the same length as the number of grouping variables. 
  ...
  tstar <- purrr::map(integer(B), function(x) {
    .resample_refit.cases(
      model = model,
      .f = .f,
      dat = data,
      cluster = clusters,
...
resid_bootstrap.merMod <- function(
  model,
  .f,
  B,
  .refit = TRUE,
...
wild_bootstrap.lmerMod <- function(
  model,
  .f,
  B,
  hccme = c("hc2", "hc3"),
...
reb_bootstrap.lmerMod <- function(model, .f, B, reb_type, .refit = TRUE) {
  ...
  if (reb_type == 2) {
    tstar <- .postprocess.reb2(
      t0,
      tstar,
      nbeta = length(lme4::getME(model, "beta")),
      B = B
    )
  }
