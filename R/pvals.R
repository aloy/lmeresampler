...
bootstrap_pvals <- function(
  ...
  if (!type %in% c("parametric", "residual", "case", "wild", "reb")) {
    stop(
      "'type' must be one of 'parametric', 'residual', 'case', 'wild', or 'reb'"
    )
  }
  if (!is.null(reb_type)) {
    if (!reb_type %in% 0:2) {
...
bootstrap_pvals.merMod <- function(
  model,
  type,
  B,
  resample = NULL,
  ...
  for (i in seq_along(pvals)) {
    ...
    if (trm == "(Intercept)") {
      ...
      null_mod <- update(model, as.formula(paste(". ~ . -", trm)))
    }
    ...
    if (type == "case") {
      boot_ts <- purrr::map_dbl(
        gen_data,
        ~ refit_to_newdf(model, newdata = .x, .f = extract_t)
      )
    } else {
