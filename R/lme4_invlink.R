...
Gamma_simfun <- function(
  ...
  if (any(wts != 1)) {
    message("using weights to scale shape parameter")
  }
...
gamma.shape.merMod <- function(object, ...) {
  ...
  Dbar <- dev / length(y)
  structure(
    list(alpha = (6 + 2 * Dbar) / (Dbar * (6 + Dbar)), SE = NA), # FIXME: obtain standard error
    class = "gamma.shape"
  )
}

#' @importFrom statmod rinvgauss
inverse.gaussian_simfun <- function(
  object,
  nsim,
  ftd = fitted(object),
  wts = weights(object)
...
negative.binomial_simfun <- function(
  object,
  nsim,
  ftd = fitted(object),
  wts = weights(object)
