  ...
  if (is.null(mod.rand)) {
    f1 <- factory(
      function(mod.fixd, mod.data, ctrl) {
        do.call(
          "lme",
          args = list(fixed = mod.fixd, data = mod.data, control = ctrl)
        )
      }
    )
...
refit_lme <- function(ystar = NULL, model, .f) {
  if (!is.null(ystar)) {
    refits <- purrr::map(ystar, function(y) {
      updated.model(model = model, new.y = y)
    })
  }
...
extract_zlist.lme <- function(model) {
  ...
  Z <- purrr::map(seq_along(re.form), function(i) {
    model.matrix(formula(model$modelStruct$reStr)[[i]], data = model$data)
  })
  ...
  Zlist <- purrr::map(seq_along(Z), function(i) {
    purrr::map(Z[[i]], function(col) split(col, model$group[, i]))
  })
  names(Zlist) <- names(Z)
  # }
  Zlist
...
.Zbstar.combine.lme <- function(bstar, Zlist) {
  zbstar_list <- purrr::map(seq_along(Zlist), function(e) {
    z.e <- Zlist[[e]]
    b.e <- bstar[[e]]
    purrr::map(seq_along(z.e), function(j) {
      unlist(mapply("*", z.e[[j]], b.e[, j], SIMPLIFY = FALSE))
    })
  })
