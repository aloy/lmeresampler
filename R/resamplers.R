      ...
      if (i == length(cluster) & resample[i]) {
        ...
        if (resample[i]) {
          ...
          res <- purrr::map_dfr(
            res,
            function(df) {
              # ldply to purrr map from list to df
...
.resample_refit.cases <- function(model, .f, dat, cluster, resample, .refit) {
  ...
  if (inherits(model, "lmerMod")) {
    ...
    stop(
      "model class must be one of 'lme', 'lmerMod', 'glmerMod', or 'glmmTMB'"
    )
  }
  tstar
}
...
.resample.cgr <- function(
  ...
  if (rbootnoise != 0) {
    ustar[[1]][["(Intercept)"]] <- ustar[[1]][["(Intercept)"]] +
      rnorm(nclusters, 0, rbootnoise * sde)
  }
...
.resample.wild <- function(
  ...
  # Calc. bootstrap y
  if (hccme == "hc2") {
    v <- (1 / sqrt(1 - .hatvalues)) * mresid
  }
  if (hccme == "hc3") {
    v <- (1 / (1 - .hatvalues)) * mresid
  }
...
.resample.reb <- function(Xbeta, Ztlist, Uhat, estar.vec, flist, levs) {
  ...
  for (i in seq_along(resamp_e_grps)) {
    ...
    estar[target_units] <- sample(
      estar.vec,
      size = length(target_units),
      replace = TRUE
    )
  }
